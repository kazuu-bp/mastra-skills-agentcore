...
`cd agent && npm run dev` でMastra Studioを開き、GUI経由で追加することも可能です。

もしくは、`agent/src/mastra/public/workspace/.agents/skills` にスキルを追加してください。

### AgentCore Runtime 用

S3バケット（デプロイ時に自動作成）にスキルを配置してください。

```shell
# バケット名はデプロイ時の出力（SkillsBucketName）から取得
aws s3 cp ./skills/ s3://<SkillsBucketName>/skills/ --recursive
```
...
# Mastraディレクトリに移動して起動
cd agent && npm run dev:agentcore
```
