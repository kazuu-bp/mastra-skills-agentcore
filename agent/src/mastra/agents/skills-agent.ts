...
export const skillsAgent = new Agent({
  ...
  instructions: `
    ...
    **終わらないように頑張ってください！**
    ...
    『スキルはworkspace/.agents/skills/を確認してから実行します！
    必要な依存関係はインストール済みとして作業を開始します。対話形式コマンドはタイムアウトになるため実行しません。
    お渡しするファイルはworkspace/outputs/に出力します。それでは、途中で終わらないように頑張ります！』
`,
